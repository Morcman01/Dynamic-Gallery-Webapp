...
| Security Misconfiguration (Mismatched Token) | The application sent out the required security token, which works fine on localhots but breaks on https due to forced lowercase by the transport protocol | Exact-case HTTP header comparison passed all local testing but failed under production HTTP/2, where header names are mandatorily lowercased per spec — fixed with a case-insensitive lookup. |
